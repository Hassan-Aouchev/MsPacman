    ...
    , WAKA_PATH{ "ms_eat_dot.wav" }
{
    ...
    //m_StateManager.ChangeState(std::make_unique<NormalPlayerState>());
}

void PlayerComponent::Update(float deltaTime)
{
    //if (!m_Lives.IsAlive())
    //    GetSubject()->NotifyObservers(GetOwner(), "ActorDied");

    m_StateManager.Update(deltaTime);

}

void PlayerComponent::ChangeState(std::unique_ptr<PlayerState> newState)
{
    m_StateManager.ChangeState(std::move(newState));
}
...
void PlayerComponent::ProcessMovement(float deltaTime)
{
    ...
    const float forgivenessThreshold = 0.05f; // Allows input slightly before reaching an intersection
